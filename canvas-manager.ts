...
export class CanvasManager {
  ...
  async createFinalSummary(summary: MeetingSummary, metadata: any): Promise<void> {
    try {
      ...
      // Add header section
      await this.addSummaryHeader(summaryFrame, metadata);
      ...
      // Add progress check
      await this.addProgressSection(summaryFrame, summary.progress);
      
      // Add key decisions
      if (summary.decisions.length > 0) {
        await this.addDecisionsSection(summaryFrame, summary.decisions);
      }
      
      // Add action items
      if (summary.actions.length > 0) {
        await this.addActionsSection(summaryFrame, summary.actions);
      }
  ...
  private async addSummaryHeader(parent: FrameNode, metadata: any): Promise<void> {
    // Title
  ...
  private async addProgressSection(parent: FrameNode, progress: any): Promise<void> {
  ...
  private async addDecisionsSection(parent: FrameNode, decisions: string[]): Promise<void> {
    const section = figma.createFrame();
    section.layoutMode = 'VERTICAL';
    section.counterAxisSizingMode = 'FIXED';
    section.primaryAxisSizingMode = 'AUTO';
  ...
  // Add actions section
  private async addActionsSection(parent: FrameNode, actions: string[]): Promise<void> {
    const section = figma.createFrame();
    section.layoutMode = 'VERTICAL';
    section.counterAxisSizingMode = 'FIXED';
    section.primaryAxisSizingMode = 'AUTO';
  ...
  private async addSummarySection(parent: FrameNode, title: string, content: string, icon: string = ""): Promise<void> {
    const section = figma.createFrame();
    section.layoutMode = 'VERTICAL';
    section.counterAxisSizingMode = 'FIXED';
    section.primaryAxisSizingMode = 'AUTO';
